...
export interface IHabitsClient {
    ...
    getUserHabits(userId: string): Promise<UserHabitsDto>;

    createHabitList(userId: string, command: NewListCommand): Promise<HabitListDto>;

    createHabit(userId: string, command: NewHabitCommand): Promise<HabitListDto>;

    updateHabit(userId: string, command: UpdateHabitCommand): Promise<HabitDto>;

    deleteHabit(userId: string, command: DeleteHabitCommand): Promise<Habit>;

    deleteHabitList(userId: string, command: DeleteListCommand): Promise<HabitList>;

    updateHabitList(userId: string, command: UpdateListCommand): Promise<HabitList>;

    getCompletions(userId: string, days: number): Promise<CompletionDto[]>;

    completeHabit(userId: string, command: CompleteHabitCommand): Promise<CompletionDto>;
}
...
export interface IUsersClient {
    ...
    getUserFromEmail(email: string): Promise<UserDto>;

    createUser(command: CreateUserCommand): Promise<User>;

    getJPUserFromEmail(email: string): Promise<JPUserDto>;
...
export class UsersClient implements IUsersClient {
    ...
    getJPUserFromEmail(email: string): Promise<JPUserDto> {
        let url_ = this.baseUrl + "/jpusers/{email}";
        if (email === undefined || email === null)
            throw new Error("The parameter 'email' must be defined.");
        url_ = url_.replace("{email}", encodeURIComponent("" + email));
        url_ = url_.replace(/[?&]$/, "");
